...
interface EthicsBoardItem {
  id: string;
  text: string;
}

function generateId() {
  return `eb_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
}

export default function EthicsBoardItemsEditor() {
  const [items, setItems] = useState<EthicsBoardItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    fetch("/api/admin/ethics-board-items")
      .then((res) => res.json())
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/admin/ethics-board-items", {
        method: "PUT",
    ...
    <div>
      <div className={css.header}>
        <div className={css.headerLeft}>
          <h1>จัดการจรรยาบรรณคณะกรรมการ</h1>
          ...
          {items.map((item, i) => (
            <div key={item.id} className={css.item}>
              ...
              <div className={css.itemActions}>
                <button className={css.iconBtn} onClick={() => handleMove(i, -1)} disabled={i === 0} title="ย้ายขึ้น">
                  <ArrowUpOutlined />
                </button>
                <button className={css.iconBtn} onClick={() => handleMove(i, 1)} disabled={i === items.length - 1} title="ย้ายลง">
                  <ArrowDownOutlined />
                </button>
                <button className={`${css.iconBtn} ${css.iconBtnDanger}`} onClick={() => handleRemove(i)} title="ลบ">
                  <DeleteOutlined />
                </button>
              </div>
            </div>
